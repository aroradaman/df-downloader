...
class downloader() :
	...
	def echo_zone(self) :
		...
		echo_server.bind((self.config['server_ip'],self.config['server_port']+1))
		while True :
			data, address = echo_server.recvfrom(1500)
			echo_server.sendto(self.md5(data),address)

	def open_echo_zone(self) :
		threading.Thread(target=self.echo_zone,args=()).start()
	...
	def check_if_online(self) :
		...
		for peer in self.config['friends'] :
			check_client.sendto('!@#$%^',(peer,self.config['server_port']+1))
